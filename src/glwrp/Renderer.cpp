...
Renderer::Renderer(VertexArray* vao, Shader* shader) : shader(shader),
                                                       vao(vao),
                                                       drawMode(DrawMode::VERTEX_ARRAY)
{}
...
Renderer::Renderer(VertexBuffer* vbo, IndexBuffer* ibo, VertexBufferLayout* layouts, unsigned int numberOfLayouts,
                   Shader* shader) : shader(shader),
                                     ...
                                     ibo(ibo),
                                     drawMode(
                                             DrawMode::VERTEX_AND_INDEX_BUFFER),
                                     numberOfLayouts(numberOfLayouts),
                                     layouts(layouts)
{}
...
void Renderer::draw()
{
    if (drawMode != DrawMode::VERTEX_AND_INDEX_BUFFER)
        throw "you have to use the constructor 'Renderer::Renderer(VertexBuffer* vbo, IndexBuffer* ibo, Shader* shader)' to use that function or use 'Renderer::draw(unsigned int numberOfVertices)' instead.";
    ...
    temp_va.bind();
    shader->bind();
    glCall(glDrawElements(GL_TRIANGLES, ibo->numberOfVertices, GL_UNSIGNED_INT, nullptr));
    shader->unbind();
    temp_va.unbind();
}
...
void Renderer::draw(void* offset)
{
    ...
    temp_va.bind();
    shader->bind();
    glCall(glDrawElements(GL_TRIANGLES, ibo->numberOfVertices, GL_UNSIGNED_INT, offset));
    shader->unbind();
    temp_va.unbind();
}
...
void Renderer::draw(GLenum mode)
{
    ...
    temp_va.bind();
    shader->bind();
    glCall(glDrawElements(mode, ibo->numberOfVertices, GL_UNSIGNED_INT, nullptr));
    shader->unbind();
    temp_va.unbind();
}
...
void Renderer::draw(void* offset, GLenum mode)
{
    ...
    temp_va.bind();
    shader->bind();
    glCall(glDrawElements(mode, ibo->numberOfVertices, GL_UNSIGNED_INT, offset));
    shader->unbind();
    temp_va.unbind();
}
